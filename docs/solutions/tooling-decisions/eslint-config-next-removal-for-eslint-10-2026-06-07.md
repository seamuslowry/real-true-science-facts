...
- Use `reactHooks.configs.flat.recommended` (not `reactHooks.configs.recommended.rules`) — the flat-config-native export self-registers the plugin and uses the object-map format flat config requires.
- `eslint-config-prettier` must come last; it disables stylistic rules from earlier configs that conflict with Prettier.
- `next-env.d.ts` is added to `globalIgnores` — it is auto-generated by Next.js and should not be linted. The original config only had three entries; this fourth entry is intentional.
